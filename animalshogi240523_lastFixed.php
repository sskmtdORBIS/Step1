    ...
    public function viewBoard(){
        ...
        for($i=1;$i<=4;$i++){
            for($j=1;$j<=3;$j++){
                ...
                if($piece!=null){
                    $pieceString = $pieceString.$piece->owner.$piece->type[0];
                } else {
                    $pieceString = $pieceString."__";
                }
            }
            $pieceString = $pieceString."<br>";
        }
        return $pieceString;
    }
...
*/
    ...
    {
        //!(!is_int($row) or !is_int($col) or $row < 1 or $row > 4 or $col < 1 or $col > 3)（メモ：整数ではないまたは盤のマス内ではない）を返す
        return !(!is_int($row) or !is_int($col) or $row < 1 or $row > 4 or $col < 1 or $col > 3);
    }
...
$board = new Board;
echo $board->viewBoard();
echo "<br>";
$board->printRiskAt(2,2);
